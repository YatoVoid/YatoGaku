...
- Caveat, recorded honestly rather than glossed over: its current content pairs Japanese with Vietnamese (and includes HSK/Chinese content YatoGaku does not need). It targets Vietnamese speakers learning Japanese, not English speakers. This means later content key results will need to either add English gloss data alongside the existing Vietnamese data, or swap the translation layer, while keeping the underlying kanji dataset, quiz engine, and SM-2 SRS logic intact. This is a real, scoped follow up task for a later key result, not a blocker to vendoring the project now: the engine and data structure are the valuable, hard to rebuild part; the translation strings are the easy part to extend.
